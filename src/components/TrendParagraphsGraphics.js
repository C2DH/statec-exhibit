...
class TrendParagraphsGraphics extends React.Component{
  ...
  render() {
    ...
    console.info('TrendParagraphsGraphics rendered.', this.props.currentParagraph?.idx)
    const x = (d) => scaleX(d.time)
    const y = (d) => scaleY(d.value)
    return (
      <g className="TrendParagraphsGraphics" transform={`translate(${marginLeft}, ${marginTop})`}>
        {paragraphs.map((p, i) => (
            <LinePath
              data={p.values}
              key={`linepath-${i}`}
