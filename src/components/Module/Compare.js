  ...
  return(
    transitions.map(({ item, props: { y, ...rest }, key }, index) => (
        <animated.div
          ...
          <div className="dtc ph2" style={{width: 200 }}>{item.label}</div>
          {item.values.map(({v}, j) => (
            <div className="dtc ph2 tr" key={`${key}-${j}`} style={{
              opacity: selectedIndex === j? 1.0: 0.35,
              border: `1px solid ${selectedIndex === j && index === 0 ? 'black': 'transparent'}`
            }}>{t('number', {n: v})}</div>
          ))}
        </animated.div>
    ))
...
const Compare = ({
  ...
  const closestDatumIdx = insertIdx === 0
    ? 0
    : (
      Math.abs(currentTime.diff(timePoints[insertIdx])) >  Math.abs(currentTime.diff(timePoints[insertIdx - 1]))
      ? insertIdx - 1
      : insertIdx
    )
  const rows = useMemo(() => {
    return groups.map((g) => {
      return {
        ...
        values: dataset.values.map(d => ({
          t: moment(d.t, dataset.timeFormat), v: d[g]
        })),
      }
    })
  }, [dataset, groups]).sort((a, b) => (b.values[closestDatumIdx].v - a.values[closestDatumIdx].v))
  ...
  return (
    ...
    <div className="dt w-100">
      <div className="dt-row">
        ...
        {timePoints.map((t, i) => (
          <div className="dtc ph2 tr bb" key={i} style={{
